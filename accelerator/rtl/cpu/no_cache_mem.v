...
module no_cache_mem #(
  ...
  // DMA Interface (256-bit read-only, two adjacent 128-bit lines)
  input                       dma_req_valid,
  output                      dma_req_ready,
  input [WORD_ADDR_BITS-1:0]  dma_req_addr,
  output reg                  dma_resp_valid,
  output reg [2*`MEM_DATA_BITS-1:0] dma_resp_data
);
  ...
  // DMA Port Logic
  assign dma_req_ready = 1'b1; // Always ready in this simple model

  wire [`ceilLog2(DEPTH)-1:0] dma_ram_addr;
  assign dma_ram_addr = dma_req_addr[WORD_ADDR_BITS-1:`ceilLog2(WORDS)];

  always @(posedge clk) begin
    if (reset)
      dma_resp_valid <= 1'b0;
    else if (dma_req_valid && dma_req_ready) begin
      dma_resp_valid <= 1'b1;
      dma_resp_data <= {ram[dma_ram_addr + 1'b1], ram[dma_ram_addr]};
    end else
      dma_resp_valid <= 1'b0;
  end
